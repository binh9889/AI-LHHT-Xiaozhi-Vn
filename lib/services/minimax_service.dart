...
class MiniMaxService {
  ...
  Future<String> sendMessage(
    ...
    try {
      if (forceNewConversation) {
        ...
        print('MiniMaxService: 强制创建新Cuộc trò chuyện，清除历史');
      }
      ...
      print('MiniMaxService: Mô hình = $model');
      print('MiniMaxService: Cuộc trò chuyện ID = $sessionId');
      ...
      if (response.statusCode == 200) {
        ...
        if (choices != null && choices.isNotEmpty) {
          String content =
              choices[0]['message']?['content'] as String? ?? 'Không có phản hồi';
  ...
  Stream<String> sendMessageStream(
    ...
    try {
      if (forceNewConversation) {
        _sessionHistory.remove(sessionId);
        print('MiniMaxService Stream: 强制创建新Cuộc trò chuyện，清除历史');
      }
      ...
      print('MiniMaxService Stream: Mô hình = $model');
      print('MiniMaxService Stream: Cuộc trò chuyện ID = $sessionId');
      ...
      if (streamedResponse.statusCode != 200) {
        ...
        print('MiniMaxService Stream: Lỗi响应体 = $responseBody');
        throw Exception(
  ...
  void clearConversation(String sessionId) {
    _sessionHistory.remove(sessionId);
    print('MiniMaxService: 已清除Cuộc trò chuyện $sessionId 的历史');
  }

  /// Clear all conversation histories.
  void clearAllConversations() {
    _sessionHistory.clear();
    print('MiniMaxService: 已清除所有Cuộc trò chuyện历史');
  }
