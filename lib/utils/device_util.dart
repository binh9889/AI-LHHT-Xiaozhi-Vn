...
class DeviceUtil {
  ...
  static const String STABLE_MAC_KEY = "xiaozhi_stable_mac";
  static const String STABLE_CLIENT_ID_KEY = "xiaozhi_stable_client_id";
  ...
  // 生成唯一的Cuộc trò chuyệnID
  static String generateConversationId() {
    return const Uuid().v4();
  }
  /// Stable locally-administered MAC-like identity used by Xiaozhi protocol.
  static Future<String> getStableMacAddress() async {
    final prefs = await SharedPreferences.getInstance();
    final cached = prefs.getString(STABLE_MAC_KEY);
    if (cached != null && cached.isNotEmpty) return cached;

    final seed = await getDeviceId();
    final digest = sha256.convert(utf8.encode(seed)).bytes;
    final bytes = digest.take(6).toList();
    // Locally administered + unicast.
    bytes[0] = (bytes[0] | 0x02) & 0xFE;
    final mac = bytes.map((b) => b.toRadixString(16).padLeft(2, '0')).join(':');
    await prefs.setString(STABLE_MAC_KEY, mac);
    return mac;
  }

  /// Stable UUID-style Client-ID, persisted across app restarts.
  static Future<String> getStableClientId() async {
    final prefs = await SharedPreferences.getInstance();
    final cached = prefs.getString(STABLE_CLIENT_ID_KEY);
    if (cached != null && cached.isNotEmpty) return cached;
    final clientId = const Uuid().v4();
    await prefs.setString(STABLE_CLIENT_ID_KEY, clientId);
    return clientId;
  }
